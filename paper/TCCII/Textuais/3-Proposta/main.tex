...
% R. Análise perceptiva do travamento de vídeos (QoE final) usando \textit{round-robin}, seleção aleatória, MM 6s, MM 60s, MM 600s (todos com técnicas de interpretação de dados obsoletos).
...
\item[$T_{stale}$]: Tempo médio de obsolescência de dados de carga de todos os servidores;
...
\item[$C$]: Coeficiente de suavização para relativização de obsolescência alta e baixa (definido heuristicamente mediante experimentação);
\end{enumerate}
...
\Procedure{AtualizarDistribuicao}{$LM_i, LD_i, T_i, AR_i$}
    ...
    \State $T \leftarrow \frac{1}{n} \sum_{i=1}^{n} (T_{atual} - T_i)$ \Comment{Tempo médio de obsolescência}
