...
O \textit{Docker} utiliza o sistema de \textit{cgroups} (grupos de controle) do \textit{kernel} \textit{Linux} para implementar limites de recursos e realizar o monitoramento de uso de \textit{CPU}, memória e leitura de disco. Especificamente, o experimento utiliza \textit{cgroups v2}, que oferece uma interface unificada e simplificada para o gerenciamento de recursos. Através dos arquivos de sistema disponibilizados pelo \textit{cgroups}, é possível ler diretamente as métricas de consumo de memória e taxa de leitura de disco de cada \textit{contêiner} em tempo real.
...
Cada \textit{contêiner} de servidor MPEG-DASH foi configurado com um \textit{bind mount} que mapeia o diretório específico do servidor no \textit{host} para o diretório interno do \textit{contêiner}. Esta técnica permite que cada servidor acesse apenas seu próprio conjunto de arquivos, simulando a exclusividade de conteúdo encontrada em servidores de borda de uma CDN real. O uso de \textit{bind mounts} também evita a necessidade de copiar os arquivos para dentro das imagens \textit{Docker}, reduzindo o tamanho das imagens e facilitando a atualização do conteúdo sem reconstruir os \textit{contêineres}.
...
\begin{enumerate}
    ...
    \item Obtenção das estatísticas de leitura através do arquivo \texttt{/sys/fs/cgroup/io.stat}, extraindo o total de \textit{bytes} lidos (\texttt{rbytes});
    ...
    \item Normalização das métricas em relação aos limites configurados para o \textit{contêiner}, gerando valores entre 0 e 1;
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Gráfico de \textit{Bitrate} (\textit{Bitrate Timeline})}: Mostra a taxa de transferência média (\textit{throughput}) medida pelo \texttt{dash.js} para cada segmento baixado, em kilo\textit{bits} por segundo (kbps). Este gráfico utiliza uma escala logarítmica no eixo Y para melhor visualização da variação de \textit{bitrate} ao longo do tempo.
...
O \textit{broker} é executado em um \textit{contêiner} \textit{Docker} dedicado e expõe a porta 1883 para comunicação MQTT não criptografada. A configuração é realizada através de um arquivo \texttt{nanomq.conf} montado como volume no \textit{contêiner}, permitindo ajustes finos de parâmetros como o número máximo de conexões simultâneas e o tamanho dos \textit{buffers} de mensagens.
...
O painel de monitoramento foi implementado como uma página \textit{HTML} estática que utiliza a biblioteca \textit{Chart.js} para renderizar gráficos em tempo real. A página é servida através de um \textit{contêiner} \textit{nginx} e se comunica com a API de métricas através de requisições HTTP periódicas utilizando \textit{JavaScript}.
...
Cada subdiretório \texttt{/data/server-X} foi então montado como um volume no \textit{contêiner} correspondente através da diretiva \texttt{volumes} no arquivo \texttt{docker-compose.yml}:
