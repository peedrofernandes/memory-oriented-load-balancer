...
\begin{enumerate}
    ...
    \item Obtenção das estatísticas de leitura de memória secundária através do arquivo \texttt{/sys/fs/cgroup/io.stat}, extraindo o total de \textit{bytes} lidos (\texttt{rbytes});
...
\begin{enumerate}
    ...
    \item \textbf{Monitoramento de Memória}: Estratégia proposta neste trabalho, que utiliza as métricas de consumo de memória e disco publicadas pelos servidores para calcular probabilidades de seleção baseadas na carga atual e na interpretação de dados potencialmente obsoletos, conforme descrito no Capítulo~\ref{cap:Proposta}.
\end{enumerate}
