...
A qualidade de reprodução apresentou algumas variações. O \textit{Round-Robin} alternava entre \textit{720P} e \textit{1080P}. A Seleção Aleatória mostrou maior variabilidade, com transições ocasionais para \textit{480P} e \textit{240P} no início. O algoritmo de monitoramento de memória manteve qualidade mais consistente, predominantemente em \textit{1080P} (intervalo de 6s) ou alternando entre \textit{720P} e \textit{1080P} (intervalo de 30s).
...
Estes resultados têm implicações práticas diretas para decisões de implantação em sistemas reais. Em ambientes altamente padronizados, como \textit{data centers} modernos com provisionamento automatizado e hardware uniforme, estratégias simples podem ser suficientes e preferíveis devido à menor complexidade.
...
A qualidade de reprodução apresentou diferenças dramáticas entre estratégias. Calculando a qualidade média ponderada (onde \textit{144P}=1, \textit{240P}=2, \textit{360P}=3, \textit{480P}=4, \textit{720P}=5, \textit{1080P}=6), observa-se:

No Cenário 1, \textit{Round-Robin} apresentou qualidade média de aproximadamente 4,0 (equivalente a \textit{480P}), com alta variância. O algoritmo de monitoramento de memória (6s) alcançou 5,7 (quase \textit{1080P} puro), com baixa variância. A diferença de 42,5\% representa a distinção entre qualidade moderada instável e qualidade excelente consistente.

No Cenário 3, a divergência foi ainda maior. \textit{Round-Robin} apresentou qualidade média de 1,2 (pouco acima de \textit{144P}), essencialmente colapsando para a qualidade mínima. O algoritmo proposto (6s) manteve 5,3 (entre \textit{720P} e \textit{1080P}). Esta diferença de 341,7\% é a distinção entre um sistema que falha em sua função básica e um que oferece experiência de alta qualidade.
