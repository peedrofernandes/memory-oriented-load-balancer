...
Um resultado particularmente significativo e promissor para aplicações práticas é a robustez do algoritmo quando operando com dados relativamente desatualizados. O desempenho mantido com intervalo de atualização de 30 segundos valida o conceito de \textit{Load Interpretation} adaptado das equações propostas, onde as chegadas esperadas durante o período de obsolescência são incorporadas no cálculo probabilístico de seleção de servidores.
...
\ul{Muito bem! Considerações Gerais: Há necessidade de reformulação da apresentação dos resultados. Da forma que está, é muito longa e com muitas subdivisões. Sugiro apresentar os resultados por métrica, ou outra forma que permita a comparação direta entre os cenários e estratégias de balanceamento de forma fluída e direta. Recomendo a criação de um Apêndice para dizeres via utlização de um link, onde ficará o código fonte, arquivos de dados e scripts de configuração, etc..., para possibilitar a reprodutibilidade dos experimentos. Há a possibilidade de colocares tudo em um arquivo zip e integrares no pdf usando o latex, como alternativa. Deixo um texto como referência como comentário aqui.}