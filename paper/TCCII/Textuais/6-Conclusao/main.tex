...
Cada \textit{branch} contém configurações pré-ajustadas (arquivos \texttt{docker-compose.yml}, parâmetros de gerador de carga, configurações de servidores) correspondentes exatamente às condições experimentais descritas no Capítulo~\ref{cap:Resultados}. Para reproduzir um cenário específico, basta fazer \textit{checkout} da \textit{branch} correspondente e executar os \textit{scripts} de orquestração fornecidos.
