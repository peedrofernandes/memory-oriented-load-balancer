...
\begin{frame}{Fluxo de Múltiplas Camadas de Cache em CDNs}
    \centering
    \includegraphics[
        width=0.9\textwidth,
        keepaspectratio
    ]{imagens/cdn-caching-structure.png}
    
    \vspace{4pt}
    \footnotesize Fluxo de requisições percorrendo múltiplas camadas de \textbf{cache} em uma CDN.
\end{frame}
...
\begin{itemize}
    \item \textbf{NGINX}: Utiliza algoritmos como \textit{Round-Robin} e \textit{Least Connections}; não monitora \textbf{uso de memória} em tempo real para decisões de balanceamento \cite{nginxAlgorithmsEvaluationYuhong2022, nginxDoc}.

    \item \textbf{HAProxy}: Balanceador eficiente e amplamente usado, mas sem mecanismo nativo de decisão baseado em memória \cite{haproxy}.

    \item \textbf{Docker Swarm}: Experimentos adicionam influência da memória no balanceamento, porém focados apenas no ecossistema Docker, sem foco em servidores de distribuição de conteúdo \cite{dockerswarmmemory2018}.

    \item \textbf{Algoritmos Dinâmicos em Cloud}: Abordagens que combinam múltiplas métricas (CPU, memória, largura de banda) para lidar com \textit{Flash Crowds}, mas sem especialização para gargalos de memória em CDNs \cite{novelWeightAssignmentLoadBalancingAlgorithmForCloudApps2023}.
    
    \item \textbf{Revisões Sistemáticas}: Estudos apontam que poucos algoritmos usam memória e disco como \textbf{parâmetros principais}, e nenhum deles de forma exclusiva \cite{webServerPerformanceImprovement2021Review}.
    
    \item \textbf{Streaming de Vídeo (iSCON)}: Balanceamento baseado em \textbf{largura de banda}, replicando conteúdo popular, mas sem considerar memória como gargalo central \cite{dynamicLoadBalancingForEfficientVideoStreamingService2015}.
    
    \item \textbf{Servidores de Vídeo Distribuídos}: Medem carga pelo número de \textbf{streams ativas}, ignorando gargalos de recursos de baixo nível como memória e disco \cite{dataAllocationAndDynamicLoadBalancingForDistributedVideoStorageServer1999}.
\end{itemize}
\end{frame}
...
\begin{frame}{Pseudocódigo de Seleção de Servidor}
    \begin{algorithm}[H]
...
\begin{frame}{Cenários de Teste}
    ...
    \begin{itemize}
        \item \textit{Timeout} de requisição: 30s;
        \item Catálogo com 12 vídeos e 6 resoluções.
    \end{itemize}

    \vspace{6pt}
    \textbf{Cenários Avaliados}
    \begin{itemize}
        \item \textbf{Cenário 1}: 100 usuários, servidores \textbf{heterogêneos};
        \item \textbf{Cenário 2}: 100 usuários, servidores \textbf{homogêneos};
        \item \textbf{Cenário 3}: 1000 usuários, servidores \textbf{heterogêneos};
        \item \textbf{Cenário 4}: 1000 usuários, servidores \textbf{homogêneos}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Qualidade de Vídeo -- Cenário 3}
    \centering
    \includegraphics[
        width=0.85\textwidth,
        keepaspectratio
    ]{imagens/outputs/quality-charts/scenario-3.png}
    
    \vspace{4pt}
    \footnotesize Evolução da qualidade de vídeo no Cenário 3 (1000 usuários, servidores heterogêneos).
\end{frame}

\begin{frame}{Bitrate por Segmento -- Cenário 3}
    \centering
    \includegraphics[
        width=0.85\textwidth,
        keepaspectratio
    ]{imagens/outputs/bitrate-charts/scenario-3.png}
    
    \vspace{4pt}
    \footnotesize Evolução do \textit{bitrate} por segmento no Cenário 3.
\end{frame}

\begin{frame}{Travamentos -- Cenário 3}
    \centering
    \includegraphics[
        width=0.85\textwidth,
        keepaspectratio
    ]{imagens/outputs/stall-charts/scenario-3.png}
    
    \vspace{4pt}
    \footnotesize Eventos de travamento (\textit{stalls}) no Cenário 3 para as diferentes estratégias.
\end{frame}
