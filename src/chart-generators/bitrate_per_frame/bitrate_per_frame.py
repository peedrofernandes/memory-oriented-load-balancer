...
def create_bitrate_charts(config: Dict[str, Any], output_path: str) -> None:
    ...
    # Consistent strategy colors across all scenarios
    strategy_order = ['round-robin', 'random-selection', 'memory-monitoring-30s', 'memory-monitoring-6s']
    ...
    for scenario, scenario_series in all_datasets.items():
        ...
        for i, canonical in enumerate(ordered):
            ...
            # Convert bps to kbps
            y_values = [float(v) / 1000.0 for v in bitrate_values]
            if y_values:
            ...
            eps = (scenario_min_positive * 0.1) if (scenario_min_positive and scenario_min_positive > 0) else 0.1
        ...
        # Use absolute values on y-axis, no scientific notation
        ax = plt.gca()
        ...
        if 'y_axis_limits' in chart_settings:
            try:
                ...
                lower = max(0.1, float(y_min))
                upper = float(y_max)
                if lower >= upper:
                    upper = lower * 10
                plt.ylim(lower, upper)
            except (ValueError, TypeError):
                upper = scenario_max * 1.1 if scenario_max > 0 else 1000
                lower = max(0.1, scenario_min_positive * 0.8 if scenario_min_positive and scenario_min_positive > 0 else 0.1)
                if lower >= upper:
                    upper = lower * 10
                plt.ylim(lower, upper)
        else:
