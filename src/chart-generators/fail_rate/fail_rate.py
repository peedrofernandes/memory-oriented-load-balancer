#!/usr/bin/env python3
"""
Fail Rate Chart Generator

Reads a JSON mapping of scenario -> fail rate value and generates a bar chart.
X-axis: scenario names, Y-axis: numeric values.

Supports optional chart_settings in the JSON for appearance control.

Usage:
    python fail_rate.py --config config.json > chart.png
    python fail_rate.py --config config.json | tee chart.png
...
{
  "data": { "scenario-a": 0.12, "scenario-b": 0.21 },
  "chart_settings": {
    "figure_size": [12, 8],
    "colors": ["#1f77b4"],
    "dpi": 300,
    "title": "Fail Rate by Scenario",
    "x_label": "Scenario",
    "y_label": "Fail Rate",
    "y_axis_limits": [0, 1]
  }
...
def extract_data_and_settings(config: Dict[str, Any]) -> Tuple[Dict[str, float], Dict[str, Any]]:
    # Support two shapes: either the file is the mapping itself, or it has "data" and "chart_settings"
...
def create_bar_chart(data: Dict[str, float], settings: Dict[str, Any]) -> None:
    def get_category_from_name(name: str) -> str:
        # expect keys like "scenario-1-round-robin"; use the suffix after the last '-'
        if '-' in name:
    ...
    # Category color mapping (can be overridden in chart_settings)
    ...
    plt.ylabel(settings.get('y_label', 'Fail Rate'), fontsize=12, fontweight='bold')
    plt.title(settings.get('title', 'Fail Rate by Scenario'), fontsize=14, fontweight='bold', pad=20)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a bar chart of fail rate by scenario and output to stdout",
...
Examples:
  ...
  python fail_rate.py --config config.json | tee chart.png
    ...
    try:
        ...
        print("Generating fail rate chart...", file=sys.stderr)
